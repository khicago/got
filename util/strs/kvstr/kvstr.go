...
func (kv KVStr) ReflectTo(target any, queryOption *QueryOption) (extra map[string]string, err error) {
	...
	var reflector reflecto.FieldHandler = func(fieldCtx reflecto.FieldContext) error {
		str, ok := kvMap.TryGet(fieldCtx.Name, queryOption)
		if !ok {
			return nil
		}

		converted, err := strs.Conv2PlainType(str, fieldCtx.Type)
		if err != nil {
			return err
		}

		val := reflect.ValueOf(converted)
		fieldCtx.Value.Set(val)
		delete(kvMap, fieldCtx.Name) // todo: check this
		return nil
	}

	if err = reflecto.ForEachField(target, reflector,
